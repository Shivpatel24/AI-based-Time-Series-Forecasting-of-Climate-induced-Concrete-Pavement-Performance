...
def preprocess_file(file_path, station_id):
    ...
    df = df[['ds', 'AirTemp', 'Wind', 'Tdp', 'Solar', 'CESR']]
    df = df.sort_values('ds').drop_duplicates(subset='ds')
    df = df.rename(columns={'CESR': 'y'})
    df['ID'] = station_id
    return df
...
def train_global_model(df,
                       model_save_path="/home/bel/Desktop/Shiv_SRIP/Saved Models/global_model.np",
...
def evaluate_on_new_data(new_file_path, model_path):
    ...
    df_new = df_new[['ds', 'AirTemp', 'Wind', 'Tdp', 'Solar', 'CESR']]
    df_new = df_new.sort_values('ds').drop_duplicates(subset='ds')
    df_new = df_new.rename(columns={'CESR': 'y'})
...
if __name__ == "__main__":
    data_folder = "/home/bel/Desktop/Shiv_SRIP/ATenLoc/Test/Alb30/h400"
    df_all = load_all_data(data_folder)
    model, _ = train_global_model(df_all,
                                  model_save_path="/home/bel/Desktop/Shiv_SRIP/ATenLoc/Prophet/model/400_30_cesr.np",
